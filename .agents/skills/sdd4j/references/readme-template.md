...
the specs, not a source of truth** — the per-SDD4J capability `package-info` docs (and the system doc) stay
authoritative; the README only renders a human-readable view *over* them, the repo-altitude analog
of how `javadoc` publishes each SDD4J capability's `package-info`. Add it only when a human on-ramp helps; a
one-SDD4J capability scratch project needs none.

It has two slices, handled oppositely:

- **Generated** — everything derivable from the specs (charter, vision, the SDD4J capability map, and a Mermaid
...
- **Hand-maintained** — only what *no spec covers*, so it cannot drift from one: project-local
  ...
  domain, intent) that `/sdd4j new` reads.

Rules for filling it in:

- **Projection, not source (the generated block).** Never hand-write capability, charter, vision, or SDD4J capability content **into the generated block** — that lives in the specs and is projected here; hand-typing it there recreates the drift SDD4J bans (*"the gap is read, not stored"*).
- **Doubles as the seed.** The hand-written sections outside the markers are the free-form inception **seed** `/sdd4j new` (no argument) reads to bootstrap vision + specs. Rich vision/intent framing is welcome here — it *seeds* the distilled `## Vision`, it isn't the canonical line. SDD4J reads the seed, never rewrites it; once specs exist they are the source of truth.
- **The generated slice is fenced** by `<!-- sdd4j:generated:start -->` / `<!-- sdd4j:generated:end -->`. `apply` replaces only what is between the markers and preserves everything outside. **No markers → `apply` leaves the README untouched** — a README is opt-in, never forced.
- **Generated content** = the system doc's **Charter** + **Vision** (if present) + a **SDD4J capability map** (each SDD4J capability name, its `>` responsibility one-liner, and a link to its `package-info`) + a **Mermaid `## Components` diagram**. Read from the per-SDD4J capability docs and the system doc — the *"mark it generated, regenerate it"* rule made concrete at repo altitude.
- **Components diagram — projection, never inference.** Render only the *declared* wiring in the system doc's `## Components` (allowed calls + integration events) as a Mermaid graph: nodes are SDD4J capabilities, edges the declared directed relationships. **Never infer edges by scanning code** — that is drift-prone discovery, not projection. No `## Components` (a one-SDD4J capability system) → nodes only, or omit the diagram. Basic Mermaid `flowchart`/`graph` syntax (version-stable, corpus-dense); delegate diagram style to `/mermaid` or `/bce-diagrams`.
...
<!-- sdd4j:generated:start — projection of the specs; do not edit; `apply` regenerates from the system doc + per-SDD4J capability package docs -->
...
## Components
<!-- projection of the system doc's ## Components wiring; nodes = SDD4J capabilities, edges = declared calls/events; never inferred from code -->
```mermaid
flowchart LR
  checkout --> inventory
